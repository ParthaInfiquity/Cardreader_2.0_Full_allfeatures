...
#include "timer.h"
#include "timer_if.h"
#include "stdbool.h"                       //added by brij
#include "button_if.h"
#include "stdlib.h"
#include "Melody.h"
#include "wdt_if.h"
...
#define GET_ON_GAME_ON                 	2
#define GET_ON_GAME_END                	3
#define GET_SSID_PWD					4
#define POST_BLOB_REQUEST               2
//*****************************************************************************
//                 GLOBAL VARIABLES -- Start
//*****************************************************************************

char DefaultSSID[50] = "DERBI PRE INCUBATION New";                //This is temperory please remove this
char DefaultPWD[50] = "IeDc_123";
char SW_Version[50] = "Card reader ver1.0.0";
...
extern volatile int TicketCounter;   //ticket counter
extern int TicketMultiplier;
char index = 0;
short MotorFlag;

unsigned char nuidPICC[4];
extern char dbSSID[50]  ;          //Read from Database
...
extern unsigned int CardUidNo;
extern int TicketWon;
extern char HttpBuf[127];
extern  char apikey[10];
extern char ReaderId[13];
extern char TransactionId[50];
int counterSsid = 5;
#if defined(ccs)
  ...
  else
  {
          ...
          LCDSelectLine(1);
                ...
                for (i = 0; i < 4; i++)
                {
                        ...
                        Var.byte[i] = uid.uidByte[i];
                }


       CardUidNo = Var.ConvNum ;
       print_num((unsigned int)CardUidNo);
  }
...
// To print the MAC address & Software details
void UARTStartupDisplay()
{
        ...
        Report("HW Part No: 0x");
        for(i = 0; i < 6; i++)
        {
                Report("%x", macAddressVal[i]);
                sprintf(ReaderId+(i*2), "%x", macAddressVal[i] );
        }
        ReaderId[12] = '\0';
                Report("\n\r");

        Report("Software Version: %s\n\r", SW_Version);
        Report("Deafult SSID: %s\n\r", DefaultSSID);
        Report("Deafult PWD: %s\n\r", DefaultPWD);
}
...
unsigned char digitalRead(unsigned char ipPin)
{
        ...
        return(GPIO_IF_Get(28, GPIOA3_BASE, GPIO_PIN_4));
}
void digitalWrite(unsigned char opPin, unsigned char Level)
{
        GPIO_IF_Set(28,GPIOA3_BASE, GPIO_PIN_4, Level);
}
...
{
         GPIO_IF_Set(8, GPIOA1_BASE, GPIO_PIN_0, 1);  //RFID power
   ...
   PCD_Init();
    ...
    GPIO_IF_Set(30, GPIOA3_BASE, GPIO_PIN_6, 1); //LCD Power
    GPIO_IF_Set(0, GPIOA0_BASE, GPIO_PIN_0, 1);  //Backlight
    GPIO_IF_Set(7, GPIOA0_BASE, GPIO_PIN_7, 0);  //BUZZER
    GPIO_IF_Set(6, GPIOA0_BASE, GPIO_PIN_6, 0);  //LED
        lcd_init();
...
void TimerRefIntHandler(void)
{
    ...
    GPIO_IF_Set(9, GPIOA1_BASE, GPIO_PIN_1, !GPIO_IF_Get(9, GPIOA1_BASE, GPIO_PIN_1));
    ++TicketCounter;

    //LCDClear();
    //LCDSelectLine(1);
    //moveCursorLeft(2);
    //print_num(((TicketCounter+1)/2)*TicketMultiplier);
}
...
void TimerBaseIntHandler(void)
{
    ...
    gameStartFlag++;
    //MAP_GPIOPinWrite(GPIOA0_BASE,GPIO_PIN_5,(MAP_GPIOPinRead(GPIOA0_BASE, GPIO_PIN_5) ^ GPIO_PIN_5));
    GPIO_IF_Set(5, GPIOA0_BASE, GPIO_PIN_5, !GPIO_IF_Get(5, GPIOA0_BASE, GPIO_PIN_5));
    //Message("IN ISR Base\n");
}

void MotorInputHandler()
{
    if(!MotorFlag && Coinflag)
    {
        Report("In Button ISR falling edge\n");
        Timer_IF_Start(g_ulRefBase,TIMER_A, 180);                       // Start timer for ticket dispense pulse
        MotorFlag =1;
    }
    else if(MotorFlag)
    {
        Report("In Button ISR Raising edge\n");
         Timer_IF_Stop(g_ulRefBase, TIMER_A);
         MotorFlag = 0;
    }
}
void ClearHttpBuf(void)
{
	int i;
	for(i=0;i<128;i++)
		HttpBuf[i] = 0;
...

{
    ...
    int iRetVal = 0;
    double   credit_bal , ticket_bal,creditToPlay, TicketBal;
   // int TicketMultiplier ;
    int HttpBufCount,apikeyCount;
    bool FreePlayFlag , CanPlayGameFlag;
    ...
    Button_IF_Init(MotorInputHandler , NULL);
    ...
    if(CallHttpPost(POST_BLOB_REQUEST)< 0)
    {
    	SlStop();
    	WDT_IF_Init(NULL,80000000 * 10);
    	LOOP_FOREVER();
    }
    // logic to remove "" from Httpbuf to get api key
    for(HttpBufCount=0,apikeyCount=0;HttpBuf[HttpBufCount];HttpBufCount++,apikeyCount++)
    {
    	if(HttpBuf[HttpBufCount] == '"')
    	{
    		HttpBufCount++;
    	}
    	apikey[apikeyCount] = HttpBuf[HttpBufCount];
    }
    ClearHttpBuf();
    if(CallHttpGet(GET_CREDIT) < 0 )
    {
        SlStop();
        WDT_IF_Init(NULL,80000000 * 10);
        LOOP_FOREVER();
    ...
    creditToPlay  = (double)GetOBjJSON(HttpBuf , GET_CREDIT_TO_PLAY);
    LCDClear();
     while(1)
	{
		GPIO_IF_Set(6, GPIOA0_BASE, GPIO_PIN_6, 1);             //Turn On the Green LED and let it be ON throughout the game
		LCDSelectLine(0);
		PrintFloat(creditToPlay);
		print(" CREDITS");
		LCDSelectLine(1);
		print("SCAN CARD 2 PLAY");
		MAP_SPIEnable(GSPI_BASE);
		Message("\n Calling RC522 main loop");
		//Main loop of the card reader where card is scanned and the UID(Physical) is obtained
		...
		if(Coinflag == 1)
		{
			...
			Melody(2);
			ClearHttpBuf();
			if(CallHttpGet(GET_USER_INFO)< 0 )					//First GET call to obtain ticket balance of the user
			{
					SlStop();
					WDT_IF_Init(NULL,80000000 * 10);
					LOOP_FOREVER();
			}
			GPIO_IF_Set(6, GPIOA0_BASE, GPIO_PIN_6, 0);             			//Reset the Green LED
			credit_bal = (double)GetOBjJSON(HttpBuf ,CURRENT_CREDIT_BAL);		//Parse current cred bal
			ticket_bal = (double)GetOBjJSON(HttpBuf , CURRENT_TICKET_BAL);		//Parse current ticket bal
			GPIO_IF_Set(6, GPIOA0_BASE, GPIO_PIN_6, 1);             			//Turn On the Green LED and let it be ON throughout the game
			FreePlayFlag = (bool)GetOBjJSON(HttpBuf , FREEPLAY_FLAG);			//Get Freeplay Flag
			CanPlayGameFlag = (bool)GetOBjJSON(HttpBuf , CANPLAY_FLAG);			//Get CanPlay flag, this is based on ticket bal
			TicketMultiplier = (int)GetOBjJSON(HttpBuf , TICKET_MULTIPLIER);	//This should be used at end to calculate number of tickets
            if(CanPlayGameFlag == (bool)0x01 || FreePlayFlag == (bool)0x01)
            {
				if(FreePlayFlag == (bool)0x01)
				{
						LCDClear();
						LCDSelectLine(0);
						print("    FREEPLAY    ");
						LCDSelectLine(1);
						print("    02:30:10    ");								//Where do we get this number from?
				}
				else
				{
						LCDClear();
						LCDSelectLine(0);
						print("CRED.BAL TICKETS");
						LCDSelectLine(1);
						PrintFloat(credit_bal);
						print("   ");
						print_num((int)ticket_bal);
						delay(1000);
						delay(1000);
						delay(1000);
				}
				ClearHttpBuf();
				if(CallHttpGet(GET_ON_GAME_ON) < 0)
				{
						SlStop();
						WDT_IF_Init(NULL,80000000 * 10);
						LOOP_FOREVER();
				...
				GetOBjJSON(HttpBuf , TRANSACTION_ID);
				GPIO_IF_Set(05, GPIOA0_BASE, GPIO_PIN_5, 0);                    // Set logic low and complete the pulse // set logic high on coin pulse gpio
				Timer_IF_Start(g_ulBase, TIMER_A, 600);                         // start the timer for oneshot pulse
				while(!(gameStartFlag == NO_OF_PULSES));                                   // Wait until Pulse width time
				Timer_IF_Stop(g_ulBase, TIMER_A);                               // Set logic low and complete the pulse
            ...
            {
				GPIO_IF_Set(6, GPIOA0_BASE, GPIO_PIN_6, 0);             		//Turn Off the Green LED
				LCDClear();
				LCDSelectLine(0);
				print("CARD CREDIT LOW");
				LCDSelectLine(1);
				print(" RECHARGE CARD");
				delay(1000);
				delay(1000);
        ...
        else if(!counterSsid)													//See if its time to recheck SSID and PWD
        {
			ClearHttpBuf();
			if(CallHttpGet(GET_SSID_PWD) < 0)									//Get SSID and PWD from the server
			{
				SlStop();
				WDT_IF_Init(NULL,80000000 * 10);
				LOOP_FOREVER();
			}
			GetOBjJSON(HttpBuf , SSID_PWD);										//Get SSID and PWD from the buffer
			if((!strcmp(storedSSID, dbSSID)) && (!strcmp(storedPWD, dbPWD)))	//if no change, nothing to do
			{
				//Do nothing here we do not need to write
			}
			else
			{
				FileIo(1, dbSSID, dbPWD);										//If changed, update the EEPROM
				iRetVal = FileIo(0, storedSSID, storedPWD);						//Read back to update the values for next compare
			}
			counterSsid= 100;													// We can increase this to a bigger value like a timer for reading SSID and PWD
        }
        else
        {
        	counterSsid--;													//Keep reducing until its time to read again
        }																	//End of coin flag if else

        if(MotorFlag == 1)
        {
			Button_IF_EnableInterrupt(SW2);
			while(MotorFlag)
				Melody(1);													//Play the melody while ticket is counted
			Button_IF_EnableInterrupt(SW2);
			LCDClear();
			GPIO_IF_Set(9, GPIOA1_BASE,GPIO_PIN_1, 0);                      //switch off the gpio of ticket dispense
			GPIO_IF_Set(7, GPIOA0_BASE,GPIO_PIN_7, 0);                      // turn off buzzer
			Report("Tickets won = %d\n",((TicketCounter+1)/2));				//Add 1 for a probable correction of ticket count /2 because we would have counted 2 edges
			LCDClear();
			LCDSelectLine(0);
			print("Tickets Won!");
			LCDSelectLine(1);
			TicketWon = ((TicketCounter+1)/2)*TicketMultiplier ;			//Calculate tickets won
			print_num(TicketWon);
			ClearHttpBuf();
            if(CallHttpGet(GET_ON_GAME_END) < 0 )
            {
                SlStop();
                WDT_IF_Init(NULL,80000000 * 10);
                LOOP_FOREVER();
            ...
            TicketCounter = 0;
            GPIO_IF_Set(6, GPIOA0_BASE, GPIO_PIN_6, 0);                                         //Turn off the LED
            delay(1000);
            delay(1000);
            delay(1000);
            LCDClear();
            LCDSelectLine(0);
            print("TICKET BALANCE");
            LCDSelectLine(1);
            TicketBal = (double)GetOBjJSON(HttpBuf , UPDATED_TICKET_BALANCE);
            PrintFloat(TicketBal);
            delay(1000);
            delay(1000);
            LCDClear();
        }
...
void CallbackForSSIDPWD(char *ssidname, char *ssidpwd)
{
	char storedSSID1[50] = "DERBI PRE INCUBATION New";              	//This is the first default SSID and password that will be used
	char storedPWD1[50] = "DERBIPINEW";
	FileIo(0, storedSSID, storedPWD);
	//if((storedSSID == NULL) || (resetssid))
	if(storedSSID[0] == NULL)
	{
			FileIo(1, storedSSID1, storedPWD1);                             //Need to debug further
			strcpy(ssidname, storedSSID1);
			strcpy(ssidpwd, storedPWD1);
			FileIo(0, storedSSID, storedPWD);
			return;
	}
	  strcpy(ssidname, storedSSID);
	  strcpy(ssidpwd, storedPWD);
}
